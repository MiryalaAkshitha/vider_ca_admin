...
function ClientInvoiceDetails() {
  ...
  const getBalanceTotalAmount = (invoicesList: any) => {
    const invoicesum = invoicesList.reduce((total: any, invoice: any) => total + (+invoice.pgpayment + +invoice.servicepayment), 0);
    setBalanceAmount((+state.amount + +previouscredits) - +invoicesum);
  }

  const getInvoicedAmount = (invoicesList: any) => {
    return invoicesList.reduce((total: any, invoice: any) => total + (+invoice.pgpayment + +invoice.servicepayment), 0);
  }
