...
function BankDetails({ result }) {
  const dispatch = useDispatch();
  const { billingEntity, bankDetails } = useSelector(selectEstimate);
  const [bank, setBank] = useState("");

  const { data, isLoading }: ResType = useQuery(
    ["billing-entity-bank-accounts", { billingEntityId: result?.billingEntity?.id }],
    getBankAccounts,
    {
      onSuccess: (res: any) => {
        if (result?.bankDetails && result?.bankDetails?.bankName !== '') {
          let bankAccount = res.data.find((item: any) => item.accountNumber == result?.bankDetails?.accountNumber);
          setBank(bankAccount?.id);
