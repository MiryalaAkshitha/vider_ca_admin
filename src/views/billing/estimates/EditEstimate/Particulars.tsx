...
function Particulars({ result }) {
  const state = useSelector(selectEstimate);
  const dispatch = useDispatch();
  const [open, setOpen] = useState(false);  

  useEffect(() => {
    if (result?.particulars && result?.particulars.length > 0) {      
      result?.particulars.forEach((data: any, index: any) => {
        setTimeout((i) => {
          const tax: any = TAXES.find((tax: any) => tax.value == i?.gst);
          i.gst = tax;
          dispatch(handleExistingParticular(i));
        }, 500, data);
      });
    }
  }, []);
